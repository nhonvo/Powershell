            ...
            try
            {
                switch (lAlias)
                {
                    ...
                    case "rebuild-tui":
                        ...
                        var projFile = Path.Combine(Config.GetProfileRepoRoot(), "apps", "agytui", "AgyTui", "AgyTui.csproj");
                        if (!File.Exists(projFile)) projFile = Path.Combine(Config.GetProfileRepoRoot(), "csapp", "AgyTui", "AgyTui.csproj");
                        ...
                        if (buildExit == 0)
                        {
                            try
                            {
                                var projDir = Path.GetDirectoryName(projFile) ?? Path.Combine(Config.GetProfileRepoRoot(), "apps", "agytui", "AgyTui");
                                var binDir = Path.Combine(projDir, "bin", "Release", "net9.0");
                                var distDir = Path.Combine(projDir, "dist");

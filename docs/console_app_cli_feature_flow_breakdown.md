...
| :--- | :--- | :--- |
| **Numeric Selection (`1`..`9`)** | Instantly selects item index `N-1` or types index number into linear prompt | Fast-path option jump |
| **Navigation Keys (`↑` / `k`, `↓` / `j`)** | Moves selection highlight up/down in memory buffer without full screen clear | Cursor home `\x1b[H` + line clear `\x1b[K` |
| **Selection Activation (`Enter`, `Space`)** | Executes highlighted action, opens child sub-view, or submits input | Trigger handler in `IScreenView` / Command Router |
| **Back / Exit (`Esc`, `q`, `0`, `b`)** | Returns to parent menu level or exits current screen cleanly | Exit navigation result `NavigationAction.Exit` |
| **Fuzzy Search Trigger (`/`, `Ctrl+F`)** | Enters single-column live search mode with active filter text in footer prompt | Real-time `SystemHelper.IsFuzzyMatch` filter |
| **Word Delete (`Ctrl+W`, `Backspace`)** | Deletes preceding word in search buffer via `DeletePreviousWord` helper | Buffer trimming `trimmed[..lastSpace]` |
| **Mouse Wheel Scroll** | Scrolls active item list up by 3 lines (`ScrollUp`) or down by 3 lines (`ScrollDown`) | Tracking via `ScreenChrome.ReadKeyWithMouse` |

### 0.2 Standardized UI Layout & Mockup Rebuilding Pipeline
```mermaid
flowchart TD
    A[Screen View Init] --> B[Hide Cursor \x1b[?25l]
    B --> C[Position Cursor Home \x1b[H]
    C --> D[Render Top List Items 100% Header-Free for Mouse Copying]
    D --> E[Render Line Clear \x1b[K at end of each row]
    E --> F[Render Footer Bar: Title + Breadcrumbs + Nav Hints]
    F --> G[Flush Memory Buffer via Single Console.Write Call]
    G --> H[Show Cursor \x1b[?25h & Await Standardized Key Input]
```

...
function Add-AgyAccount {
    ...
    # Copy root configuration and token files (exclude directories and credentials)
    $credentialsFiles = @("google_accounts.json", "oauth_creds.json", "state.json")
...
function Remove-AgyAccount {
    ...
    $targetDir = "$($script:AgyAccountPrefix)$AccountName"
    if (-not (Test-Path $targetDir)) {
        Write-Error "Account '$AccountName' does not exist at $targetDir."
        return
    }

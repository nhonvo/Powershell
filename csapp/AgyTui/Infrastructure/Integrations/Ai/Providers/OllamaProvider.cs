...
public static class OllamaProvider
{
    ...
    public static void PersistDefaultModel(string model)
    {
    ...
    public static bool IsPortListening(int port) => IPGlobalProperties.GetIPGlobalProperties().GetActiveTcpListeners().Any(e => e.Port == port);

    public static bool IsOllamaRunning() => IsPortListening(11434);
    ...
    {
        EnsureOllamaServer();
        var selectedModel = !string.IsNullOrEmpty(model) ? model : OllamaDefaultModel;
        AiProcessRunner.RunInteractive("ollama", new[] { "run", selectedModel });
    }

    public static void InitializeOllamaServer() => EnsureOllamaServer();

    public static void SetOllamaModel(string? modelName)
    {
        if (!string.IsNullOrWhiteSpace(modelName))
        {
            PersistDefaultModel(modelName.Trim());
        }
    }
